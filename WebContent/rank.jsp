...
<body>
	...
	/* if (request.getAttribute("name") != null) {
		...
		game_name = request.getParameter("name");
	} else {
		System.out.println("게임이름을 받아오지 못했습니다.");
		
	}
	//리뷰 페이지 번호받기
	if (session.getAttribute("r_pageNumber") != null) {
		r_pageNumber = Integer.parseInt(session.getAttribute("r_pageNumber").toString());
	...
	if (session.getAttribute("rlist") == null) {
		...
		rlist = (ArrayList<ReviewVO>)session.getAttribute("rlist"); //리스트 생성
		System.out.println("리뷰리스트 블러오기 성공, r_pagenumber : " + r_pageNumber);
	}
	 */
	 
	 //마지막... 게임 이름, 페이지 넘버, 게임별 리뷰 리스트
...
if(rlist != null) {
	System.out.println("리뷰 리스트 받아옴");
}
 rlist=rdao.getListGame_name(name, begin, end);
...
<div class="container" style="margin-top:100px">
    ...
    <tr>
        <td style="font-size:20px;font-weight: bold; font-style: italic;width:20%;" width="50px">
    ...
    <tr>
    <td colspan="9">
<div style="margin-left:-1000px; width:1000px;height:500px; overflow:hidden;margin:auto"> 
<iframe style="position:relative; top:-485px; left:-20px;" width="980px" height="860px" src="http://www.gevolution.co.kr/app/?svc=649619#game_score" scrolling="no"></iframe>
</div>
    </td>
    </tr>
			...
			<table border=0 class="table">
				...
				<tbody>
					<tr style="height: 7px; font-size: 13px;">
						...
						r=r*10;%>
						<td style="text-align: left; font-weight: 800; font-size: 13px;">
							<div id="starrating" style="CLEAR: both;	PADDING-RIGHT: 0px;	PADDING-LEFT: 0px;	BACKGROUND: url(images/icon_star2_16.png) 0px 0px;	FLOAT: left;	PADDING-BOTTOM: 0px;	MARGIN: 0px;	WIDTH: 80px;	PADDING-TOP: 0px;	HEIGHT: 16px;">
				    		<p id="starrating1" style="PADDING-RIGHT:0px;	PADDING-LEFT:0px;	BACKGROUND: url(images/icon_star_16.png) 0px 0px;	PADDING-BOTTOM: 0px;	MARGIN: 0px;	PADDING-TOP: 0px;	HEIGHT: 16px;WIDTH: <%=r%>%;"></p>
	...
	<div align=center>
		<div class="container">
			<ul class="pagination justify-content-center">
				...
				%>
				<li><a
					href="rank.jsp?r_pageNumber=<%=fromPage - 1%>&name=<%=name%>&img=<%=img%>&star=<%=star%>"><<</a></li>
				<%
					}
					if (r_pageNumber == 1) {
				%>
				...
				%>
				<li><a
					href="rank.jsp?r_pageNumber=<%=fromPage - 1%>&name=<%=name%>&img=<%=img%>&star=<%=star%>">Previous</a></li>
				<%
					}
				...
				%>
				<li><a href="rank.jsp?r_pageNumber=<%=fromPage - 1%>&name=<%=name%>&img=<%=img%>&star=<%=star%>"><%=i%></a></li>
				...
				<li><a
					href="rank.jsp?r_pageNumber=<%=fromPage - 1%>&name=<%=name%>&img=<%=img%>&star=<%=star%>">Next</a></li>
				<%
					} else {
				%>
				...
				<li><a href="rank.jsp?r_pageNumber=<%=fromPage - 1%>&name=<%=name%>&img=<%=img%>&star=<%=star%>">>></a></li>
				<%
					}
				%>
			</ul>
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input type="text" class="form-control" id="inputId3"
									name="game_name" value="<%=name%>" readonly>
							</div>
